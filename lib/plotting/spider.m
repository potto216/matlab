...
%http://www.mathworks.com/matlabcentral/fileexchange/22708-spider-data-tle-rng-lbl-leg-f-
...
% 				'fontsize',8);
		temp = text([cos(ang(ii)) * inc(jj) + sin(ang(ii)) * tx(jj)], ...
				[sin(ang(ii)) * inc(jj) - cos(ang(ii)) * tx(jj)], ...
				num2str(rd(rng(ii,1) + inc(jj)*diff(rng(ii,:)),roundPosition)), ...
	...
	temp = text([cos(ang(ii)) * 1.1 + sin(ang(ii)) * 0], ...
			...
			char(lbl(ii,:)),'fontsize',14);  %,'fontweight', 'bold'
...
styleCol={'-','-','-','--','--','--'};
for ii = 1:c; set(o(ii),'color',col(ii,:),'linewidth',1.5,'linestyle',styleCol{ii}); end
