...
function splineDataActive=splinedbSelectActive(metadata)
switch(nargin)
    case 0
...
if isempty(metadata.activeSpline)
    if isempty(splineData)
        error('No spline is available.  Please create one with splinedbInsert');
    else
        splineDataActive=splineData(1);
    end
else
    splineDataActive=splineData(metadata.activeSpline);
end
