function varargout = grabSWCineColorDopplerOriginal(varargin)
% GRABSWCINECOLORDOPPLERORIGINAL MATLAB code for grabSWCineColorDopplerOriginal.fig
%      GRABSWCINECOLORDOPPLERORIGINAL, by itself, creates a new GRABSWCINECOLORDOPPLERORIGINAL or raises the existing
%      singleton*.
%
%      H = GRABSWCINECOLORDOPPLERORIGINAL returns the handle to a new GRABSWCINECOLORDOPPLERORIGINAL or the handle to
%      the existing singleton*.
%
%      GRABSWCINECOLORDOPPLERORIGINAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GRABSWCINECOLORDOPPLERORIGINAL.M with the given input arguments.
%
%      GRABSWCINECOLORDOPPLERORIGINAL('Property','Value',...) creates a new GRABSWCINECOLORDOPPLERORIGINAL or raises the
%      existing singleton*.  Starting from +the left, property value pairs are
%      applied to the GUI before grabSWCineColorDopplerOriginal_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to grabSWCineColorDopplerOriginal_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help grabSWCineColorDopplerOriginal

% Last Modified by GUIDE v2.5 05-Jul-2012 11:23:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @grabSWCineColorDopplerOriginal_OpeningFcn, ...
    'gui_OutputFcn',  @grabSWCineColorDopplerOriginal_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before grabSWCineColorDopplerOriginal is made visible.
function grabSWCineColorDopplerOriginal_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to grabSWCineColorDopplerOriginal (see VARARGIN)

% Choose default command line output for grabSWCineColorDopplerOriginal
...
% UIWAIT makes grabSWCineColorDopplerOriginal w0ait for user response (see UIRESUME)
% uiwait(handles.figGrabSWCineColorDoppler);
end

% --- Outputs from this function are returned to the command line.
function varargout = grabSWCineColorDopplerOriginal_OutputFcn(hObject, eventdata, handles)  %#ok<INUSL>
...
if ~isempty(handles.settings.colorDoppler.rectangle)
    ...
    if ~isSuccessful
        shutdownObj(handles.uObj);
        errorOverride('Unable to set Color Doppler rectangle.')
    end
end
...
if ~isSuccessful
    shutdownObj(handles.uObj);
    errorOverride('Unable to get Color Doppler rectangle.')
end
guidata(hObject, handles);
end
