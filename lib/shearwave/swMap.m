...
function [fftPhase, selectedFrequency, sig,fftBinFreqToRealFreq,imgAngleFilt_rad]=swMap(imgBlock,imgBlockType,fs,temporalFrequency)

switch(imgBlockType)
    case 'time'
        imgAngle_rad=angle(imgBlock(:,1:end,1:(end-1)).*conj(imgBlock(:,1:end,2:end))); %compute the phase difference
