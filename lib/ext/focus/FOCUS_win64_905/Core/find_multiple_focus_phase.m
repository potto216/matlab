function transducer=find_multiple_focus_phase(varargin)
% find_single_focus_phase will produce an array of complex_weight shifts to focus an array defined by xdcr_array at a 
% given point (x,y,z). The input arguments are the transducer array, array parameters
% and the focal point. 
% Input parameters:
% xdcr - pointer to array aperture
% xmtx   - x coordinate of the focus points, unit: m
% ymtx   - y coordinate of the focus points, unit: m
% zmtx   - z coordinate of the focus points, unit: m
% One focus point is represented by [xtmx(i) ytmx(i) ztmx(i)]
% meduim - defines the acoustic properties of the medium contains the fields: wb, rho, c, b,wavelen, atten, ct, kappa, beta
...
% be the same as the input transducer (xdcr). The logic is that if all elements have a complex_weight of 0 at the target point,
...
Npoints = length(x);
p = ones(Npoints,1)*5e6;
Hmtx=complex(zeros(1,length(xdcr_array)));
for ie=1:array_length
    for im=1:Npoints
        my_cg = set_coordinate_grid(1,x(im),x(im),y(im),y(im),z(im),z(im));
        Hmtx(im,ie)=fnm_cw(xdcr_array(ie,:),my_cg,medium,ndiv,f0,0);
    end
end

max(max(abs(Hmtx)));
% try to do this by myself, first normalize the Hmtx,
Hmtx_norm = Hmtx;

H = Hmtx_norm;
tmpH = H*H';
excitation = H'*inv(tmpH)*p;
pw = inv(tmpH)*p;

for ie=1:array_length
    for je = 1:array_height
        xdcr_array(ie,je).amplitude = abs(excitation(ie));
        xdcr_array(ie,je).phase = angle(excitation(ie));
    end
end
