...
if false
    %%
    figure; plot([0:(length(impulse_response)-1)]/fs*1e6,impulse_response)
    xlabel('\musec')
    ylabel('amplitude')
    title('Impulse Response')
    
    figure; plot([0:(length(excitation)-1)]/fs*1e6,excitation)
    xlabel('\musec')
    ylabel('amplitude')
...
if false
    %%
    phantom_positions_mm=dd.phantom_positions*1000;
    lpBright=[dd.phantom_amplitudes>6];
   lp=[dd.phantom_amplitudes>2 & ~lpBright]; figure; plot3(phantom_positions_mm(lp,1),phantom_positions_mm(lp,2),phantom_positions_mm(lp,3),'.') 
   hold on
    plot3(phantom_positions_mm(lpBright,1),phantom_positions_mm(lpBright,2),phantom_positions_mm(lpBright,3),'r.') 
