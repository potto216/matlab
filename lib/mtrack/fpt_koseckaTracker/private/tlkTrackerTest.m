%     good(:,nbri) = goodfeat; %The good feature good(:,n) is going from featx(:,n-1) to featx(:,n)
%feature points in an image are stored as a column vector frame numbers are stored as columns 
%     featx(:,nbri) = xttnew(1,:)';
%     featy(:,nbri) = xttnew(2,:)';
%     featq(:,nbri) = Qtt;
function [featx,featy,featq,good]=tlkTrackerTest(frames,interlaced,bPlot,bMovieout)
...
bPlot = 0;          % Set to 1 to get graphical output
bMovieout = 0;      % Can be used only if bPlot activated
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% num = 120;
% [frames] = uread('11-48-19.rf',[0:num],'frameFormatComplex',true);
...
for nbri=(index+1):duration;
    ...
    good(:,nbri) = goodfeat; %The good feature is going from featx(:,n-1) to featx(:,n)
    featx(:,nbri) = xttnew(1,:)';
    featy(:,nbri) = xttnew(2,:)';
    featq(:,nbri) = Qtt;
...
warning(' featx and featy appear switched, so switching back');
feattmpy=featy;
featy=featx;
featx=feattmpy;
