...
t_rc=vBase_rc(:,3:end)-vBase_rc(:,1:end-2);     %Equation (4)
n_rc=[0 -1; 1 0]*t_rc;
nNorm_rc=n_rc./([1;1]*sqrt(sum(n_rc.^2,1)));
%According to the paper the length between any two elements in the tangent direction should be 
%smaller than the depth of the band (normal direction).  One approximation
%is to use the average length between adjacent snake elements, which often
%leads to an edge length of several pixels
%is 

nMag=floor(d*1.2);  %the height of the normal
...
%EdgeTrack used a search space of 5
nScale=linspace(-(totalSlices-1)/2,(totalSlices-1)/2,totalSlices);  %this should be odd so zero is evaluated
