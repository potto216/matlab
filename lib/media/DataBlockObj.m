classdef DataBlockObj < handle
    ...
    %base 0
    %
    %To display region information use
    ...
    %dataBlockObj=DataBlockObj(fullfilename,@uread);
    %dataBlockObj.open('cacheMethod','all');
    %imBlock=dataBlockObj.getSlice([3 4]);  %get the third and fourth frame
    %
    %===Example of loading a sequence of 11 TIF image files in the sequence
    %100-110 with the file name 0100.tif, 0101.tif, etc.
    %fullfilepathname='full filename path'
    %dataBlockObj=DataBlockObj(fullfilepathname,@imread,'openArgs',{'%04d','tif',[100:110]});
    %dataBlockObj.open('cacheMethod','all');
    %imBlock=dataBlockObj.getSlice([3 4]);  %get the third and fourth frame
    %
    ...
    methods (Access=public)
        ...
        %
        %blockSource - the file name of the source file, the file path of a sequence of files or a 2D or 3D data
        ...
        %file,path name/matrix | data                 | apply metadataMaster information over blockSource
        %file,path name/matrix | none                 | use only the meta pulled by
        %                                               blockSource, or default to ones
        ...
        %{'uread','imread', 'matlabArray'}.  Will default to uread.  Any strings
        ...
        function dataBlockObj=DataBlockObj(blockSource,openMethod,varargin)
            ...
            p.addOptional('openMethod','uread',@(x) any(strcmp(x,{'uread', 'imread', 'matlabArray'})) || isa(x,'function_handle'));
        ...
        function open(this,varargin)
            ...
            %this sets up the data structure and reads the block
            switch(char(this.openMethod))
                case 'uread'
                    ...
                    if strcmp('all',this.cacheMethod)
                        this.blockData=this.getDataSliceFromDataSource([]);
                        %Need to compare in 4D since it could be a 3D
                        %volume
                        blockDataDimesions=ones(size(this.blockSourceDimensions));
                        blockDataDimesions(1:length(size(this.blockData)))=size(this.blockData);
                        if ~all(this.blockSourceDimensions==blockDataDimesions)
                            error('The dimensions of the block do not match the expected.');
                        end
                    end
  
                case 'imread'
                    %for imread we assume there is not useful metadata in
                    %the files
                    if isempty(this.caseFilenameDeprecated)
                        ...
                        error(['Deprecated file names are not supported for ' this.caseFilenameDeprecated]);
                    ...
                    %This will be a cell array of strings for all the files
                    %loaded and the order will correspond to the sequence
                    %in the image slice
                    this.metadata.sourceFilename=this.blockSource;                    
                    
                    this.getDataSliceFromDataSource=@(slice_base1) this.imread(this.blockSource,slice_base1,this.openArgs{:});
                    
                    if strcmp('all',this.cacheMethod)
                        this.blockData=this.getDataSliceFromDataSource([]);
                        this.blockSourceDimensions=size(this.blockData);
                    else
                        error('only a cache method of all is currently supported for imread');
                    end 
                ...
                case 'matlabArray'
                    ...
                    if ~strcmp('all',this.cacheMethod)
                        error('Only cacheMethod of "all" is supported if the source is a matlabArray');
                    end
        ...
        function [sliceBlock]=getSlice(this,slice_base1,volumeIndex,processStreamId)
            ...
            %%part handles the case when blockData=[] because size([],3)==1
        ...
        %dataBlockObj.movie([384 478],{[dataBlockObj.activeCaseName '.avi'],'w',5,{'avi', 'Uncompressed AVI'},false})
        ...
        function movie(this,varargin)
            ...
            p.addParamValue('trimBlackBorder',false,@(x) islogical(x));
            % cropIm [xmin ymin width height]
            p.addParamValue('cropIm',[],@(x) isnumeric(x) && isvector(x) && numel(x)==4);
            ...
            trimBlackBorder=p.Results.trimBlackBorder;
            ...
            for ii=range(1):range(2)
                ...
                if false
                    ...
                    im=this.getSlice(ii);
                    if trimBlackBorder
                        im=im((this.metadata.ultrasound.rf.header.ul(2)+1):(this.metadata.ultrasound.rf.header.br(2)-1), ...
                            (this.metadata.ultrasound.rf.header.ul(1)+2):(this.metadata.ultrasound.rf.header.br(1)-1));
                    elseif ~isempty(cropIm)
                        % [xmin ymin width height]
                        im=im(cropIm(2):(cropIm(2)+cropIm(4)),cropIm(1):(cropIm(1)+cropIm(3)));
                    else
                ...
                elseif ~isempty(cropIm)
                    % [xmin ymin width height]
                    axialAxis_mm=axialAxis_mm + cropIm(2)*axial_mm;
                    lateralAxis_mm=lateralAxis_mm + cropIm(1)*lateral_mm;
                else
                ...
                if ~isempty(fileCreationSettings)
                    %  vid=vwrite(vid,gca,'handle');  %Only show the image without the labels
        ...
        %This reads a sequence of images and returns it. More efficient to
        %directly write to the image seq, but we will let open do this.
        %
        %INPUT
        %Note: part of the input functionality is written to be compatible
        %with the "getDataSliceFromDataSource" wrapper function.
        %
        %filepath - The file path for where the files are located
        %
        %indexToRead - the base one index of what sliceSequences to load.
        %If it is empty then all files in the sequence will be loaded. This
        %assumes the index starts at one and runs to the length of the
        %array sliceSequence
        %
        %filenameMask - is used to create the file name with the sprintf format to accept a %d
        ...
        %zeros. This is ONLY the file name and not the file path. If no
        %file name extension is given then by default the format will be
        %appended to the end.
        %
        %sliceSequence - is an array of the order to read the slices from
        %disk. The seuqence numbers are not used to read from the data
        %since all the images will be squeezed together starting at 1.
        %
        %===OUTPUT===
        %what is returned is the image block with the third dimension
        %indicating the frame. Color images are converted to grayscale.
        %Example: dataBlockObj.imread('/filepath/',[0:10],%04d','tif',[100:110]);
        
        function imBlock=imread(this,filepath,indexToRead,filenameMask,fmt,sliceSequence)
            ...
            switch(nargin)
                ...
                case 6
                    %do nothing
                otherwise
                    error('The arguements filepath,indexToRead,filenameMask,fmt,sliceSequence all must be specified.');
            end
            
            if isempty(indexToRead)
                indexToRead = (1:length(sliceSequence));
            else
                %do nothing
            end
            ...
            imBlock=[];
            for ii=1:length(indexToRead)
                fileSequenceNumber = sliceSequence(indexToRead(ii));
                [~,~,fileExt]=fileparts(filenameMask);
                if isempty(fileExt)
                    fullFilename=fullfile(filepath,sprintf([filenameMask '.%s'],fileSequenceNumber,fmt));
                else
                    fullFilename=fullfile(filepath,sprintf(filenameMask,fileSequenceNumber));
                end
                disp(['Reading frame ' num2str(ii) ' of ' num2str(length(indexToRead)) '. Using slice number ' num2str(sliceSequence(ii)) '. ' fullFilename]);
                
                [im,imMap] = imread(fullFilename,fmt);
                if ~isempty(imMap)
                    im = ind2rgb(im,imMap);
                end
                ...
                if isempty(imBlock)
                    imBlock=zeros(size(im,1),size(im,2),length(indexToRead));
                end
                imBlock(:,:,ii)=im;
            end
        end
        ...
        %padding then make sure the mask is %03d where 3 is the number of
        %zeros
        %WARNING: The data is scaled quantized before being written out to 
        %minimze quantization error. If comparing to the original data or
        %if absolute values are important then the data needs to be rescaled
        %after loading in.
        %
        %INPUT
        ...
        function imwrite(this,filepath,filenameMask,fmt,sliceSequence)
            ...
            if any(this.blockData(:)<0)
                error('Cannot handle negative numbers');
            end
            
            maxValue=max(this.blockData(:));
            ...
            for ii=1:length(sliceSequence)
                ...
                im=uint16(floor(this.blockData(:,:,sliceSequence(ii))/maxValue*2^16));
        ...
        function plotBframes(this)
            ...
            uicontrol('Style', 'slider',...
                'Min',1,'Max',this.size(3),'Value',1,...
                'SliderStep',[1/(this.size(3)-1) 20/(this.size(3)-1)],...
                'Position', Newpos*400,...
        ...
        function framestep(this,hObj,event,ax)
            ...
            imagesc(this.getSlice(framenum,[]));colormap gray;
            title(ax,['Frame ',num2str(framenum)],'fontsize',16)
            ylabel(ax,'Depth (samples)','fontsize',16)
        end
        
        function overwriteBlockData(this, newBlockData)
            if ~all(size(this.blockData) == size(newBlockData))
                error('Cannot overwrite blocks because sizes are not equal');
            else
...
%TEST Code. This is code that could be used to verify the object is working
%correctly
%% Compare processed gait study examples
%===Test: Validate that files being written out as PNGs are loaded in with
%the same values as the original.
% Remeber the data is quantized to 16 bit so values will change but the
% dynamic range should stay about the same.
% mrvsusList= { ...    	
% fullfile('E:\dataUltrasound\FastSlowUS_svf_png_min_test\S002\S002L1_FAST1'),'standardBMode', fullfile(svFilepath,'S002L1_FAST1.mat'); ...
% %'S002L1_SLOW1','standardBMode', fullfile(svFilepath,'S002L1_SLOW1.mat'); ...
% };
% idx=1;
% d=DataBlockObj(mrvsusList{idx,1},@imread,'openArgs',{'%04d','png',[1:583]});
% d.open('cacheMethod','all');
% svf=load(mrvsusList{idx,3});
% 
% %% Compare the frames between object and image block
% if ~all(size(svf.imPngBlock ) == size(d.blockData))
%     error('Sizes are not equal')
% end
% 
% svf.imPngBlock(150:154,310:314,100)-d.blockData(150:154,310:314,100);
% maxValue=max(svf.imPngBlock(:));
% imUint16=uint16(floor(svf.imPngBlock(:,:,100)/maxValue*2^16));
% imUint16(150:154,310:314);
% err=abs(svf.imPngBlock(:)-d.blockData(:)/max(d.blockData(:))*max(svf.imPngBlock(:)));
% [maxV, maxI]=max(err);
% svf.imPngBlock(maxI)
% %maxV =  1.0486
% figure; hist(err,1111)