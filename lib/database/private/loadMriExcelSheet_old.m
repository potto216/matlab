function velocityBlock=loadMriExcelSheet(xlsFilename,xlsSheetName,velocityMeasurementsPerRegion, colOffsetBetweenVelocityBlockAndRegionLabel, startRegionLabel)
...
for ii=1:length(isRegionLabel)
    if ii==1
        velocityBlock=loadVelocityBlock(xls,regionStartRow,regionStartCol,velocityMeasurementsPerRegion,regionLabels{ii},ii,colOffsetBetweenVelocityBlockAndRegionLabel);
    else
        velocityBlock(ii)=loadVelocityBlock(xls,regionStartRow,regionStartCol,velocityMeasurementsPerRegion,regionLabels{ii},ii,colOffsetBetweenVelocityBlockAndRegionLabel);
    end
end

if ~all(diff([velocityBlock.shiftAmount])==0)
...
function velocityBlock=loadVelocityBlock(xls,regionStartRow,regionStartCol,velocityMeasurementsPerRegion,regionLabelName,regionLabelIdx,colOffsetBetweenVelocityBlockAndRegionLabel)
...
positionData_mmPerSec=xls.cell(validTable(startIdx):(validTable(startIdx)+m-1),((regionStartCol-colOffset-4-5):(regionStartCol-colOffset-4-5+2)));
velocityBlock.xlsValues=velocityData_mmPerSec;
velocityBlock.projected_mmPerSec=cell2mat(velocityData_mmPerSec(:,end));
velocityBlock.projectedAndShifted_mmPerSec=a;
velocityBlock.actual_mmPerSec=cell2mat(velocityData_mmPerSec(:,1:3));
velocityBlock.actual_mm=cell2mat(positionData_mmPerSec);
velocityBlock.regionLabelName=regionLabelName;
velocityBlock.regionStartCol=regionStartCol;
velocityBlock.shiftAmount=shiftAmount;
